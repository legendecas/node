...
const kResourceKeys = Symbol('execution_resource_keys');
const storageList = [];
let activeStorageCount = 0;
const storageHook = createHook({
  init(asyncId, type, triggerAsyncId, resource) {
    const currentResource = executionAsyncResource();
    // Value of currentResource is always a non null object
    const keys = currentResource[kResourceKeys];
    if (keys === undefined) {
      return;
    }
    resource[kResourceKeys] = keys;
    for (let i = 0; i < storageList.length; ++i) {
      resource[kResourceKeys] = storageList[i]._propagate(currentResource, keys, type);
    }
  }
});

function makeKeys(prevKeys) {
  // A unique object. Its fields are diagnostic purpose.
  const key = { id: executionAsyncId() };

  return { key, prevKeys };
}

function getStore(resource, storage) {
  let keys = resource[kResourceKeys];
...
class AsyncLocalStorage {
  ...
  disable() {
    ...
    this.enabled = false;
    activeStorageCount--;
    if (this._onPropagate) {
      // If this.enabled and has _onPropagate callback, the instance must be in storageList
  ...
  _propagate(currentResource, keys, type) {
    if (!this.enabled) {
      return;
    }
    const store = getStore(currentResource, this.#storage);
    if (!this._onPropagate(type, store)) {
      // clear the store.
      const nextKeys = makeKeys(currentResource[kResourceKeys]);
      this.#storage.set(nextKeys.key, undefined);
      return nextKeys;
    }
    return keys;
  }
  ...
  enterWith(store) {
    ...
    const nextKeys = makeKeys(resource[kResourceKeys]);
    resource[kResourceKeys] = nextKeys;
    this.#storage.set(nextKeys.key, store);
  }
  ...
  run(store, callback, ...args) {
    ...
    const resource = executionAsyncResource();
    const prevKeys = resource[kResourceKeys];
    const nextKeys = makeKeys(prevKeys);
    this.#storage.set(nextKeys.key, store);
    ...
    try {
      ...
      resource[kResourceKeys] = prevKeys;
    }
  }
  ...
  getStore() {
    ...
    const resource = executionAsyncResource();
    const keys = resource[kResourceKeys];
    if (keys === undefined) {
      return;
    }
    const store = getStore(resource, this.#storage);
    this.#storage.set(keys.key, store);
    return store;
  }
}
