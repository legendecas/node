...
function throwOnGetOrigin() {
  throw new ERR_MODULE_NO_ORIGIN();
}

/**
 * When a module is requested without a parent module, the specifier is resolved by this origin URL, as in:
 * - a main module, like `node main.mjs`, or
 * - a parent-less import, like `ShadowRealm.prototype.importValue`.
 * @returns {URL}
 */
function getOrigin() {
  return _getOrigin();
 ...
 */
function initializeESM(forceDefaultLoader = false, hasOrigin = true) {
  _forceDefaultLoader = forceDefaultLoader;
  if (!hasOrigin) {
    _hasOrigin = hasOrigin;
    _getOrigin = throwOnGetOrigin;
  }
