...
const {
  ...
  ERR_INVALID_RELATIVE_MODULE_SPECIFIER,
...
function getOrigin(isMainOrCliOption) {
  if (!isMainOrCliOption) {
    const mode = getOptionValue('--resolve-module-cwd');
    if (mode === 'throw') {
      throw new ERR_INVALID_RELATIVE_MODULE_SPECIFIER();
    }
    if (mode === 'warn') {
      process.emitWarning('Resolve module relative to cwd.');
    }
  }
  return getCWDURL().href;
}
...
async function initializeHooks() {
  ...
  const parentURL = getOrigin(/* isCliOption */true);
