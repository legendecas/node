...
function cjsPreparseModuleExports(filename, source, isMain, format) {
  ...
  if (reexports.length) {
    ...
    for (let i = 0; i < reexports.length; i++) {
      ...
      let format, url;
      try {
        ({ format, filename: resolved, url } = resolveForCJSWithHooks(reexport, module, false));
      } catch (e) {
      ...
      if (format === 'commonjs' ||
        (!StringPrototypeStartsWith(url, 'node:') && findLongestRegisteredExtension(resolved) === '.js')) {
