...
function errorValueHasCause(errVal) {
  if (typeof errVal === 'object') {
    return ObjectPrototypeHasOwnProperty(errVal, 'cause');
  }
  return false;
}

function errorValueGetCause(errVal) {
  if (typeof errVal === 'object') {
    const desc = ObjectGetOwnPropertyDescriptor(errVal, 'cause');
    return desc && desc.value;
  }
  return undefined;
}

function formatErrorCause(errVal, ctx) {
...
function formatError(err, constructor, tag, ctx, keys) {
  ...
  let hasCause = errorValueHasCause(err);
  ...
  if (!ctx.showHidden && keys.length !== 0) {
    ...
    // Hide the property that would be append to the stack
  ...
  if (hasCause) {
    stack += '\nCaused by ' + formatValue(ctx, errorValueGetCause(err), 0);
  }
