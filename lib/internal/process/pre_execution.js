...
function prepareShadowRealmExecution() {
  ...
  // Disable custom loaders and origin in ShadowRealm.
  setupUserModules(true, false);
  const {
    privateSymbols: {
...
function setupUserModules(forceDefaultLoader = false, hasOrigin = true) {
  initializeCJSLoader();
  initializeESMLoader(forceDefaultLoader, hasOrigin);
  const {
...
function initializeESMLoader(forceDefaultLoader, hasOrigin) {
  const { initializeESM } = require('internal/modules/esm/utils');
  initializeESM(forceDefaultLoader, hasOrigin);
