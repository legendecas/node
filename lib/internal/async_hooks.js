...
const promise_inited_symbol = Symbol('promiseInited');
...
function trackPromise(promise, parent) {
  ...
  promise[async_id_symbol] = asyncId;
  promise[trigger_async_id_symbol] = triggerAsyncId;
}

function promiseInitHook(promise, parent) {
  // Skip emit init if the promise is not created from a `.then` call.
  // e.g. when the promise is created from `new Promise` or an async function.
  if (parent == null) {
    return;
  }
  ...
  promise[promise_inited_symbol] = true;
...
function promiseEnqueueHook(promise) {
  const hasInited = promise[promise_inited_symbol];
  trackPromise(promise);
  // Emit init for the promises that created with `new Promise`.
  if (!hasInited) {
    const asyncId = promise[async_id_symbol];
    const triggerAsyncId = promise[trigger_async_id_symbol];
    promise[promise_inited_symbol] = true;
...
function promiseResolveHook(promise) {
  ...
  trackPromise(promise);
  const asyncId = promise[async_id_symbol];
  // Emit init for the promises that created with `Promise.resolve`.
  if (!hasInited) {
    const triggerAsyncId = promise[trigger_async_id_symbol];
    promise[promise_inited_symbol] = true;
    emitInitScript(asyncId, 'PROMISE', triggerAsyncId, promise);
