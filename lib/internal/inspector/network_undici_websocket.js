 ...
 * https://chromedevtools.github.io/devtools-protocol/1-3/Network/#event-webSocketCreated
 * @param {{ request: undici.Request }} event
 */
function onWebSocketOpen({ address, protocol, extensions }) {
 ...
 * https://chromedevtools.github.io/devtools-protocol/1-3/Network/#event-webSocketClosed
 * @param {{ request: undici.Request, error: any }} event
 */
function onWebSocketClose({ request, error }) {
...
function enable() {
  ...
  dc.subscribe('undici:websocket:socket_error', onWebSocketError);
  dc.subscribe('undici:websocket:ping', onWebSocketPing);
  dc.subscribe('undici:websocket:pong', onWebSocketPong);
}

function disable() {
  dc.unsubscribe('undici:websocket:open', onWebSocketOpen);
  dc.unsubscribe('undici:websocket:close', onWebSocketClose);
  dc.unsubscribe('undici:websocket:socket_error', onWebSocketError);
  dc.unsubscribe('undici:websocket:ping', onWebSocketPing);
  dc.unsubscribe('undici:websocket:pong', onWebSocketPong);
}
