...
const enqueueAll = makeRunHook(hooks.enqueue);
...
function update() {
  ...
  const enqueue = maybeFastPath(hooks.enqueue, enqueueAll);
  setPromiseHooks(init, before, after, settled, enqueue);
}
...
const onEnqueue = makeUseHook('enqueue');

function createHook({ init, before, after, settled, enqueue } = kEmptyObject) {
  const hooks = [];
  ...
  if (enqueue) ArrayPrototypePush(hooks, onEnqueue(enqueue));
